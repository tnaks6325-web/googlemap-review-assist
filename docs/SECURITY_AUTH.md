...
### R1 자기거래 — 제품 결정: 현 상태 수용(관리자 승인 백스톱) ✅
**결정(2026-06-29)**: 코드=VERIFIED 모델을 유지하고, **정산 관리자 승인 + 코드 상한**을 통제 수단으로 삼는다(수용된 리스크).
- 전제: 출금(SETTLE)은 자동이 아니며 관리자 승인이 게이트. 따라서 농사한 포인트도 **사람의 검토 없이는 현금화 불가**.
- 함의: **관리자 백오피스 UI(정산 큐·어뷰징 모니터링)가 백스톱의 실효성을 좌우** → 우선순위 상향 권장.
- 모니터링 권장: 동일 매장에 적립이 몰리는 리뷰어 클러스터, 단기 대량 적립 플래그(후속).
- 재검토 조건: 거래량 증가/실제 출금 발생 시 OCR 필수화(아래 원안)로 격상.
...
## 7f. 레드팀 6라운드 (관리자 백오피스/money) — 조치 내역 / 이월

역할혼동·money 권한·재처리·PII 마스킹·XSS·페이지 가드는 **non-issue 확인**(강점). 주요 발견:

| ID | 심각도 | 내용 | 상태 |
|---|---|---|---|
| F3 | HIGH | 약한 데모 관리자(`admin1234`) 시드 | ✅ 비운영 전용 시드로 게이트 |
| F2 | HIGH | 로그인 레이트리밋 XFF 스푸핑·계정 잠금 없음 | ✅ `TRUSTED_PROXY_COUNT` 기반 IP + 계정별 throttle |
| F1 | HIGH | 토큰 폴백=무귀속 payout 통제 | ✅ money 액션 **실 세션 필수**(토큰은 `ADMIN_TOKEN_MONEY=1`일 때만) + `processedBy` 감사 기록 |
| F6 | LOW | 미존재 계정 타이밍 열거 | ✅ 더미 해시로 비용 균일화(admin·owner 로그인) |
| F5 | MEDIUM | 인메모리 레이트리밋 멀티노드 | ⏭️ Redis(8라운드) |

검증: 미인증/토큰 money 차단·세션 승인 PAID(processedBy 기록)·잘못된 로그인 401·`/admin` 가드 실측.
참고: HMAC 세션은 무상태 → 로그아웃 후에도 만료(관리자 8h) 전까지 토큰 유효(설계 수용, 모니터링).

## 7g. 레드팀 7라운드 (OCR 프로바이더) — 조치 내역 / 이월

SSRF 불가·mockText 운영 게이트·키 비노출(리터럴)·ReDoS 없음·검증 구조 fail-closed는 **non-issue 확인**.

| ID | 심각도 | 내용 | 상태 |
|---|---|---|---|
| F1 | HIGH | Vision 에러 미처리 throw→500+제공자 메시지 노출 | ✅ 라우트 try/catch(502 일반응답) + 제공자 메시지 비전달 |
| F3 | MEDIUM | confidence 기본 0.8이 게이트 무력화 | ✅ DOCUMENT_TEXT_DETECTION + 부재 시 기본 0(fail-closed) |
| F5 | MEDIUM | 가맹점 부분일치(짧은 상호) 우회 | ✅ 최소 3자 + 역방향 부분일치 제거 |
| F6 | LOW | 금액 자릿수 무제한 | ✅ 자릿수 상한 |
| F4 | MEDIUM(컴플) | 사진 3자 전송 무고지 | ✅ UI 고지 문구 추가(전송 사실), ToS/처리자 문서화 권장 |

**설계 한계(명시)**: OCR 자가신고 텍스트만으로 진위 확정 불가(F5 근본). 등록 상호 포함 매칭은 완화책이며,
완전 해결은 POS/PG 거래 대사 등 out-of-band 검증 필요(R1 결정과 함께 거래량 증가 시 재검토).
