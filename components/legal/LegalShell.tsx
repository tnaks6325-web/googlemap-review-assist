...
export function LegalShell({ title, children }: { title: string; children: ReactNode }) {
  return (
    <main className="mx-auto max-w-2xl px-5 py-8">
      <Link href="/" className="text-sm text-ink-weak hover:text-ink-sub">
        ← 홈
      </Link>
      ...
      <nav className="mt-10 flex gap-4 border-t border-line pt-6 text-sm text-ink-weak">
        <Link href="/legal/terms" className="hover:text-ink-sub">이용약관</Link>
        <Link href="/legal/privacy" className="hover:text-ink-sub">개인정보 처리방침</Link>
        <Link href="/legal/reviews" className="hover:text-ink-sub">리뷰·적립 정책</Link>
      </nav>
    </main>
  );
}
