...
function errorResponse(error: unknown) {
  ...
  if (typeof candidate?.code === "string" && typeof candidate?.message === "string") {
    return err(candidate.code, candidate.message, 422);
  }
  console.error("[fine-tuning]", error);
  return err("FINE_TUNING_FAILED", "파인튜닝 작업을 처리하지 못했습니다.", 500);
}
...
export async function POST(req: Request) {
  ...
  const limited = await rateLimit(`admin:fine-tuning:${body.action}:${adminId}:${clientIp(req)}`, body.action === "start-job" ? 2 : 120, HOUR);
  if (!limited.ok) return err("RATE_LIMITED", "잠시 후 다시 시도해 주세요.", 429);
