...
export class VertexTuningConfigurationError extends Error {
  constructor(public readonly code: string, message: string) {
    super(message);
    this.name = "VertexTuningConfigurationError";
  }
}
...
function tuningConfig(env: Env = process.env) {
  ...
  if (!/^[a-z0-9][a-z0-9._-]{1,61}[a-z0-9]$/u.test(bucket)) {
    throw new VertexTuningConfigurationError(
      "VERTEX_TUNING_BUCKET_REQUIRED",
      "VERTEX_AI_TUNING_BUCKET 환경변수에 전용 Cloud Storage 버킷 이름을 등록해 주세요.",
    );
  }
  return { projectId, bucket, region: DRAFT_FINE_TUNING_REGION };
}
...
async function googleRequest(url: string, init: RequestInit, env: Env) {
  ...
  if (!response.ok) {
    ...
    throw new VertexTuningConfigurationError(
      typeof nested?.status === "string" ? nested.status : "VERTEX_TUNING_REQUEST_FAILED",
      typeof nested?.message === "string" ? nested.message : "Vertex AI 요청에 실패했습니다.",
    );
  }
  return data ?? {};
}
...
export async function createVertexTuningJob(
  input: { displayName: string; trainingGcsUri: string; validationGcsUri: string },
  env: Env = process.env,
) {
  const { projectId } = tuningConfig(env);
  return googleRequest(buildVertexTuningJobUrl(projectId), {
    method: "POST",
    ...
    body: JSON.stringify({
      ...
      supervisedTuningSpec: {
        trainingDatasetUri: input.trainingGcsUri,
        validationDatasetUri: input.validationGcsUri,
      },
    }),
  }, env);
}
...
export async function getVertexTuningJob(jobName: string, env: Env = process.env) {
  const { projectId } = tuningConfig(env);
  return googleRequest(buildVertexTuningJobUrl(projectId, jobName), { method: "GET" }, env);
}

export async function cancelVertexTuningJob(jobName: string, env: Env = process.env) {
  const { projectId } = tuningConfig(env);
  return googleRequest(`${buildVertexTuningJobUrl(projectId, jobName)}:cancel`, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }, env);
}
