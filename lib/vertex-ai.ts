...
export const DEFAULT_VERTEX_AI_MODEL = "gemini-3.5-flash";
...
export class VertexAiConfigurationError extends Error {
  constructor(
    public readonly code:
      ...
      | "VERTEX_MODEL_INVALID"
      | "VERTEX_CREDENTIALS_INVALID"
      | "VERTEX_ACCESS_TOKEN_REQUIRED",
    message: string,
  ) {
    super(message);
...
export function resolveVertexAiConfig(env: VertexEnvironment = process.env): VertexAiConfig {
  ...
  if (!projectId) {
    throw new VertexAiConfigurationError(
      "VERTEX_PROJECT_REQUIRED",
      "Vertex AI 프로젝트 설정이 필요합니다.",
    );
  }
  if (!/^[a-z][a-z0-9-]{4,28}[a-z0-9]$/u.test(projectId)) {
    throw new VertexAiConfigurationError(
      "VERTEX_PROJECT_INVALID",
      "Vertex AI 프로젝트 ID 설정을 확인해 주세요.",
    );
  }
  if (!/^(?:global|[a-z][a-z0-9-]{1,62})$/u.test(location)) {
    throw new VertexAiConfigurationError(
      "VERTEX_LOCATION_INVALID",
      "Vertex AI 리전 설정을 확인해 주세요.",
    );
  }
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]{1,127}$/u.test(model)) {
    throw new VertexAiConfigurationError(
      "VERTEX_MODEL_INVALID",
      "Vertex AI 모델 설정을 확인해 주세요.",
    );
  }
...
export function buildVertexAiUrl(config: VertexAiConfig, method: VertexAiMethod) {
  ...
  const path = [
    ...
    `${encodeURIComponent(config.model)}:${method}`,
  ].join("/");
...
export async function getVertexAccessToken(env: VertexEnvironment = process.env) {
  ...
  if (!response.ok || typeof data?.access_token !== "string" || !data.access_token) {
    throw new VertexAiConfigurationError(
      "VERTEX_ACCESS_TOKEN_REQUIRED",
      "Vertex AI 인증 토큰을 발급하지 못했습니다.",
    );
  }
  const expiresIn = typeof data.expires_in === "number" ? data.expires_in : 3_600;
  cachedAccessToken = {
    token: data.access_token,
    expiresAt: now + Math.max(60, Math.min(expiresIn, 3_600)),
    clientEmail: credentials.client_email,
  };
  return data.access_token;
}

export async function sendVertexAiRequest({
  config,
  method,
  accessToken,
  body,
  ...
  return fetchImpl(buildVertexAiUrl(config, method), {
    method: "POST",
    headers: {
...
export async function requestVertexAi(
  method: VertexAiMethod,
  body: unknown,
  timeoutMs: number,
...
export function isVertexAiConfigured(env: VertexEnvironment = process.env) {
  try {
    resolveVertexAiConfig(env);
    return Boolean(
      cleanEnvironmentValue(env.VERTEX_AI_SERVICE_ACCOUNT_BASE64),
    );
  } catch {
    return false;
  }
